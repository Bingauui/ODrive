...
void StatusLedController::update() {
    ...
    bool is_booting = axis.current_state_ == Axis::AXIS_STATE_UNDEFINED;

    if (is_booting) {
        return;
    }

    bool is_armed = axis.motor_.is_armed_;
...
static bool config_apply_all() {
    ...
    for (size_t i = 0; (i < AXIS_COUNT) && success; ++i) {
        success = encoder.apply_config(motor.config_.motor_type) && axis.controller_.apply_config() && axis.min_endstop_.apply_config() && axis.max_endstop_.apply_config() && motor.apply_config() && motor.motor_thermistor_.apply_config() && axis.apply_config();
    }
    return success;
}
...
bool ODrive::any_error() {
    return error_ != ODrive::ERROR_NONE || axis.error_ != Axis::ERROR_NONE || axis.motor_.error_ != Motor::ERROR_NONE || axis.sensorless_estimator_.error_ != SensorlessEstimator::ERROR_NONE || axis.encoder_.error_ != Encoder::ERROR_NONE || axis.controller_.error_ != Controller::ERROR_NONE;
}
...
void vApplicationIdleHook(void) {
    if (odrv.system_stats_.fully_booted) {
        ...
        uint32_t min_stack_space;
        min_stack_space = uxTaskGetStackHighWaterMark(axis.thread_id_) * sizeof(StackType_t);
        odrv.system_stats_.max_stack_usage_axis = axis.stack_size_ - min_stack_space;
...
extern "C" int main(void) {
    // 构建USB序列号的过程应该相同
    // STM内置USB引导加载程序的方式。这意味着
    // 设备在正常和DFU模式下具有相同的序列号。
    ...
    // 从NVM加载配置。这需要在system_init（）之后发生
    // 因为flash接口必须在board_init（）之前初始化
    // 因为板初始化可以取决于配置。
    size_t config_size = 0;
